...
const getCachedData = (key: string): any | null => {
  ...
  try {
    ...
    if (now - timestamp < CACHE_DURATION) {
      console.log(`✅ Cache HIT pour ${key} (${Math.round((now - timestamp) / 1000)}s)`);
      return data;
    } else {
...
const setCachedData = (key: string, data: any): void => {
  ...
  try {
    ...
    console.log(`💾 Cache SAVED pour ${key}`);
  } catch (error) {
    console.error('❌ Erreur sauvegarde cache:', error);
  }
};

// ✅ NOUVEAU : Fonction pour vider le cache manuellement
...
/**
 * ✅ OPTIMISÉ : Charge toutes les données depuis Google Sheets avec cache
 */
export const chargerDonnees = async (forceRefresh: boolean = false) => {
  ...
  try {
    ...
    console.log(`✅ Données chargées en ${duration}s et mises en cache`);
...
// ✅ Après chaque modification, vider le cache pour forcer un refresh
const invalidateCache = () => {
  clearCache();
  console.log('🔄 Cache invalidé - prochain chargement sera frais');
};
...
export const sauvegarderArticle = async (article: Article, action: 'create' | 'update'): Promise<void> => {
  ...
  invalidateCache(); // ✅ Vider le cache
  console.log('✅ Article sauvegardé:', article.nom);
  return result;
};

export const supprimerArticle = async (articleId: string) => {
  const result = await fetchJSONP(`?action=delete&table=Articles&id=${articleId}`);
  invalidateCache(); // ✅ Vider le cache
  return result;
};
...
export const supprimerContact = async (contactId: string) => {
  const result = await fetchJSONP(`?action=delete&table=Client_Fournisseurs&id=${contactId}`);
  invalidateCache(); // ✅ Vider le cache
  return result;
};

// ✅ FONCTION pour enregistrer un mouvement (16 colonnes)
export async function enregistrerMouvement(mouvement: any): Promise<boolean> {
  ...
  invalidateCache(); // ✅ Vider le cache
  console.log('✅ Mouvement enregistré avec commentaire');
...
export const sauvegarderFacture = async (facture: any) => {
  ...
  invalidateCache(); // ✅ Vider le cache
  console.log('✅ Facture sauvegardée avec commentaire');
  return result;
};
