...
export default function EditFacturePage() {
  ...
  useEffect(() => {
    const charger = async () => {
      try {
        ...
        // Trouver la facture
        const facture = data.factures.find((f: FactureResume) => f.reference === reference);
        if (!facture) {
        ...
        const lignes = data.mouvements.filter((m: Mouvement) => m.reference === reference);
        if (lignes.length === 0) {
        ...
        const client = data.clients.find((c: Contact) => c.id === clientId);
        ...
        for (const ligne of lignes) {
          const article = data.articles.find((a: Article) => a.id === ligne.articleId);
          if (article) {
            nouveauPanier.push({
              article,
  ...
  const ajouterAuPanier = (article: Article) => {
    setPanier(prev => {
      const existant = prev.find((p: LignePanier) => p.article.id === article.id);
      if (existant) {
        return prev.map((p: LignePanier) => 
  ...
  const sauvegarderModifications = async () => {
    ...
    try {
      ...
      const anciensMouvements = mouvements.filter((m: Mouvement) => m.reference === reference);
      for (const m of anciensMouvements) {
        // Remettre le stock
        const article = articles.find((a: Article) => a.id === m.articleId);
        if (article && m.type === 'vente') {
          ...
          setArticles(prev => prev.map((a: Article) => a.id === article.id ? { ...a, stock: newStock } : a));
        }
        await supprimerMouvement(m.id);
      }

      // 2. Supprimer l'ancienne facture
      const ancienneFacture = factures.find((f: FactureResume) => f.reference === reference);
      if (ancienneFacture) {
      ...
      for (const ligne of panier) {
        ...
        setArticles(prev => prev.map((a: Article) => a.id === ligne.article.id ? { ...a, stock: newStock } : a));
      }
